...
Un expediente durable exige que cada validación de `candidate.json`, su
preimagen y el intento validado tengan el mismo ID y orden, estado `passed` y
el mismo `evidenceSha256` sellado. `candidate.json` y el intento conservan la
referencia lógica fija `evidence/<id>.json` y el digest, nunca los bytes ni la
ruta de origen de la evidencia. Un candidato legado puede recibir Gate 2 para su
